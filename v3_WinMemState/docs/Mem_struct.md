...
3. addr_min이라는 이름의 저장 공간은 8bite(x64기준) 이 공간에는 오직 "숫자만" 저장됨.

    3-1 * -> addr_min 안에는 큰 숫자가 있는데 *가 있으니 이건 숫자가 아닌 어딘가의 좌표라고 인식.
    3-2 unsigned char: 만약 좌표에 도착했다면 딱 1byte 데이터만 들여다 보고, 그 안의 데이터는 0~255 사이의 숫자로만 해석.*/


주의: 포인터는 "도착하고 나서 데이터를 읽는다면" 1byte씩 읽는거지, addr_min 변수에 바로 데이터가 들어가는 것은 아님.
addr_min, addr_max는 포인터 변수이기 때문에 변수의 값은 항상 "주소"임.
...
맨 밑줄의 SYSTEM_INFO는 이 구조체의 별명. 그래서 우리는 별명을 통해 SYSTEM_INFO si 같이 간단히 변수 생성 가능.
맨 밑줄의 PSYSTEM_INFO는 이 구제체의 포인터형 별명. SYSTEM_INFO*와 동일.
...
BaseAddress(현재 구역): 아파트 단지 내의 특정 동/호수나 특정 구역의 시작 주소.


왜 나뉘어 있는데?

하나의 커다란 할당 영역(AllocationBase) 안에서도 어떤 페이지는 읽기 전용이고 어떤 페이지는 읽기/쓰기 일 수 있음.

VirtualQuery는 속성이 바뀌는 지점마다 끊어서 정보를 주는데, 이때 “지금 보고 있는 이 속성이  시작되는 지점”이 BaseAddress이다.
...
동적 코드 생성: 처음에는 데이터 저장을 위해 PAGE_READWRITE로 빌렸다가, 나중에 코드를 집어넣고 VirtualProtect 함수를 써서 PAGE_EXECUTE_READ로 권한을 바꿀 수 있음.

보안 강화(DEP): 실행이 필요 없는 데이터 영역에 누군가 코드를 심어 실행하지 못하도록 시스템이 강제로 권한을 변경하기도 함.


----------------------------------------------------------------------------------------------------------------------------------------------------------


3. MEMORY_BASIC_INFORMATION - 함수를 호출하고 나면, 선언한 이 구조체 안에 해당 영역의 정보가 담김.
